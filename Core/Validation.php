...
class Validation
{
    ...
    public $patterns = array(
        ...
        'alphanum'      => '[\p{L}0-9]+',
        'int'           => '[0-9]+',
        'float'         => '^[0-9]*\.[0-9]{2}$', // 2 floating points
        'tel'           => '[0-9+\s()-]+',
        'text'          => '[\p{L}0-9\s-.,;:!"%&()?+\'°#\/@]+',
    ...
    public function pattern($name)
    {
        if ($name == 'array') {
            ...
            if (!is_array($this->value)) {
                $this->errors[$this->name] = $this->name . ' est invalide';
            }
        } else {
            $regex = '/^(' . $this->patterns[$name] . ')$/u';
            if ($this->value != '' && !preg_match($regex, $this->value)) {
                $this->errors[$this->name] = ucfirst($this->name) . ' est invalide';
            }
        }
        return $this;
    }
    ...
    public function required(): Object
    {
        ...
        if ((isset($this->file) && $this->file['error'] == 4) || ($this->value == '' || $this->value == null)) {
            $this->errors[$this->name] = 'Champ obligatoire.';
        }
        return $this;
    }
    ...
    public function min($length)
    {
        ...
        if (is_string($this->value)) {
            ...
            if (strlen($this->value) < $length) {
                $this->errors[$this->name] = 'Le mot de passe doit avoir au moins 8 caractères';
            }
        } 
        return $this;
    }
    ...
    public function equal($value)
    {
        ...
        if ($this->value != $value) {
            ...
            $this->errors[$this->name] = 'Les mots de passe ne correspondent pas';
        }
        return $this;
    }
